...
public class BetterDynamicArrayTester {
	...
	public void testSetElementDoesNotChangeSize() {
		AudioEvent e7 = new VolumeEvent(7, 1, 1);
		threeEvents.set(0, e7);
		assertEquals(3, threeEvents.size(), "Setting an element in the array changes the size of the array");
	}
	...
	public void testSort() {
		threeEvents.set(0, e3);
		threeEvents.set(1, e2);
		threeEvents.set(2, e1);
		threeEvents.sort();
		assertEquals(e1, threeEvents.get(0), "AudioEvents were not sorted properly");
	}

	@Test
	public void testSortMixedAudioEvents() {
		AudioEvent n1 = new NoteEvent(1, 1, 1, 1);
		threeEvents.insert(2, n1);
		threeEvents.sort();
		assertEquals(n1, threeEvents.get(0), "AudioEvents were not sorted properly");
	}

	@Test
	public void testRemoveElementDoesNotChangeBackingArraySize() {
		for (int index = 4; index < 15; index++) {
			threeEvents.add(makeEvent(index));
		}
		threeEvents.remove(e1);
		threeEvents.remove(e2);
		threeEvents.remove(e3);
		assertTrue(threeEvents.toString().contains("20"));
	}

	@Test
	public void testRemoveIndexDoesNotChangeBackingArraySize() {
		for (int index = 4; index < 15; index++) {
			threeEvents.add(makeEvent(index));
		}

		for (int index = 0; index < 7; index++) {
			threeEvents.remove(index);
		}
	...
	@Test
	public void testClearDoesNotChangeBackingArraySize() {
		for (int index = 4; index < 15; index++) {
			threeEvents.add(makeEvent(index));
		}
		threeEvents.clear();
